...
describe KerbalDyn::Planetoid do
  ...
  describe 'Planetoid Library' do
    ...
    [:moho, :eve, :gilly, :duna, :ike, :jool, :laythe, :vall, :tylo, :bop].each do |factory_method|

      describe "#{factory_method.to_s.capitalize}" do

        before(:all) do
          @planet_fact = KerbalDyn::Planetoid.send(factory_method)
          @expected_name = factory_method.to_s.capitalize
        end
